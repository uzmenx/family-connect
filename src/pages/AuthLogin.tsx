...
import { Loader2, Mail, Lock, ArrowLeft, Phone } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col px-8 pt-8">
        ...
        <form onSubmit={handleLogin} className="space-y-5 flex-1">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="password" type="password" placeholder="••••••••" value={password}
                onChange={(e) => setPassword(e.target.value)} className="pl-12 h-14 bg-muted/50 border-0 rounded-xl text-base" required />
            </div>
          </div>

          <Button type="submit" className="w-full h-14 rounded-xl text-base font-semibold" disabled={isLoading}>
            {isLoading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />{t('loggingIn')}</> : t('login')}
          </Button>

          <div className="text-center">
            <Link to="/forgot-password" className="text-sm text-muted-foreground hover:text-primary transition-colors">​</Link>
          </div>
          ...
          <div className="flex gap-3">
            <Button type="button" variant="outline" className="flex-1 h-14 rounded-xl border-2" onClick={handleGoogleLogin} disabled={isGoogleLoading}>
        ...
        <div className="py-8 text-center">
          <span className="text-sm text-muted-foreground">
            {t('noAccount')}{' '}
            <Link to="/signup" className="text-primary font-semibold hover:underline">{t('register')}</Link>
          </span>
        </div>
      </div>
    </div>
