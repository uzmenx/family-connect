      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-8">
        ...
        <div className="w-full max-w-md backdrop-blur-xl bg-white/8 border border-white/20 rounded-3xl shadow-2xl p-8 space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <Button 
              ...
              disabled={isLoading}
            >
              {isLoading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Yuklanmoqda...</> : "KIRISH"}
            </Button>
          </form>

          <div className="relative py-4">
            <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-[rgba(255,255,255,0.15)]" /></div>
          ...
          >
            {isGoogleLoading ? <Loader2 className="h-6 w-6 animate-spin text-white" /> :
        ...
        <div className="mt-8 text-center">
          <span className="text-sm text-[rgba(255,255,255,0.6)]">
            Akkauntingiz yo'qmi?{' '}
            <Link to="/signup" className="text-[#22c55e] font-bold hover:text-[#16a34a] transition-colors">Ro'yxatdan o'ting</Link>
          </span>
        </div>
      </div>
    </div>
